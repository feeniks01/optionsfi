/**
 * Initialize NVDAx2 Vault (production vault with Virtual Offset security mechanism)
 * 
...
// NVDAx2 - Production vault with virtual offset security mechanism
const ASSET_ID = "NVDAx2";
...
async function main() {
    ...
    console.log("\nCreating production NVDAx2 vault with assetId:", ASSET_ID);
    ...
    try {
        ...
        console.log("\n✅ NVDAx2 Production Vault is ready!");
        console.log("\nVault Address:", vaultPda.toBase58());
        console.log("Share Mint:", shareMintKeypair.publicKey.toBase58());

        // Save addresses
        fs.appendFileSync(envPath, `\n# NVDAx2 Production Vault (Virtual Offset)\n`);
        fs.appendFileSync(envPath, `NVDAX2_VAULT_PDA=${vaultPda.toBase58()}\n`);
        fs.appendFileSync(envPath, `NVDAX2_SHARE_MINT=${shareMintKeypair.publicKey.toBase58()}\n`);
        fs.appendFileSync(envPath, `NVDAX2_VAULT_TOKEN_ACCOUNT=${vaultTokenAccountKeypair.publicKey.toBase58()}\n`);
        fs.appendFileSync(envPath, `NVDAX2_PREMIUM_TOKEN_ACCOUNT=${premiumTokenAccountKeypair.publicKey.toBase58()}\n`);
        fs.appendFileSync(envPath, `NVDAX2_SHARE_ESCROW=${shareEscrowPda.toBase58()}\n`);
    } catch (error: any) {
